...
internal sealed class CancelCallbackStateCommandHandler(IRepository<UserSession> userSessionRepository, ILogger<CancelCallbackStateCommandHandler> logger)
    :ICommandHandler<CancelCallbackStateCommand, ErrorOr<Success>>
{
    public async Task<ErrorOr<Success>> Handle(CancelCallbackStateCommand request, CancellationToken cancellationToken)
    {
        logger.LogInformation("Обработка команды {Command} ChatId: {ChatId}", nameof(CancelCallbackStateCommand), request.ChatId);
        ...
        userSession.CancelCallbackProcess();

        await userSessionRepository.UpdateAsync(userSession, cancellationToken);

        logger.LogInformation("Завершена обработка команды {Command}, заявка на обратный звонок отменена, UserSessionId: {UserSessionId}",
                              nameof(CancelCallbackStateCommand),
                              userSession.Id);
